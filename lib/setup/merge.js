...
function mergeOcodeJson(userJson = {}, templateJson = {}) {
    ...
    for (const action of ['write', 'edit']) {
        if (permission[action] && typeof permission[action] === 'object') {
            ...
            for (const requiredPath of REQUIRED_EDIT_PATHS) {
                permission[action][requiredPath] = 'allow';
            }
            for (const deniedPath of DENIED_EDIT_PATHS) {
                permission[action][deniedPath] = 'deny';
            }
        }
    }
