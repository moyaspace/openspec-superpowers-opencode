 ...
 */
function findRealOpenspec(binDir, isWin) {
    try {
        // 尝试通过 npm root -g 获取全局目录
        const npmRoot = child_process.execSync('npm root -g', { stdio: 'pipe', encoding: 'utf8' }).trim();
        if (npmRoot) {
            const candidates = [
                path.join(npmRoot, '@fission-ai', 'openspec', 'bin', 'openspec.js'),
                path.join(npmRoot, 'openspec', 'bin', 'openspec.js'),
            ];
            for (const c of candidates) {
    ...
    // 回退：从 binDir 向上推算
    // /usr/local/bin → /usr/local/lib/node_modules/@fission-ai/openspec/bin/openspec.js
    try {
        const prefix = path.resolve(binDir, '..');
        const candidates = [
            path.join(prefix, 'lib', 'node_modules', '@fission-ai', 'openspec', 'bin', 'openspec.js'),
            path.join(prefix, 'node_modules', '@fission-ai', 'openspec', 'bin', 'openspec.js'),
        ];
        for (const c of candidates) {
            if (fs.existsSync(c)) return c;
        }
...
function copyOpenspecToOrig(binDir, isWin) {
    ...
    try {
        ...
        if (content.includes('openspec shim for oso registry')) {
            ...
            if (fs.existsSync(destPath)) {
                ...
                // 文件内容损坏（旧版本留下的普通文件），删除重建
                fs.unlinkSync(destPath);
            }
            // 找到真正的 openspec（npm 全局安装的符号链接目标），创建有效备份
            const realOrig = findRealOpenspec(binDir, isWin);
            if (realOrig) {
                fs.symlinkSync(realOrig, destPath);
            }
            if (!fs.existsSync(destPath)) {
    ...
    // 保留为符号链接而非跟随它，否则相对路径 import 会解析错误
    try {
