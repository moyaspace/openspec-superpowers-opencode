...
// 安装元数据文件路径（测试可通过 _setMetaPath 覆盖）
let _metaFilePath = null;

/**
 * 获取默认的安装元数据文件路径。
 * @returns {string}
 */
function getDefaultMetaPath() {
    if (_metaFilePath) return _metaFilePath;
    const configDir = process.platform === 'win32'
        ? path.join(process.env.APPDATA || path.join(os.homedir(), 'AppData', 'Roaming'), 'openspec-superpowers-opencode')
        : path.join(os.homedir(), '.config', 'openspec-superpowers-opencode');
    return path.join(configDir, 'install-meta.json');
}
 ...
 */
function saveInstallMeta(binDir) {
    const filePath = getDefaultMetaPath();
    const dir = path.dirname(filePath);
    if (!fs.existsSync(dir)) {
        fs.mkdirSync(dir, { recursive: true });
    }
    const data = JSON.stringify({ binDir, installedAt: new Date().toISOString() }, null, 2);
    fs.writeFileSync(filePath, data, 'utf8');
}

/**
 * 读取安装元数据。
 * @returns {{binDir: string, installedAt: string}|null}
 */
function loadInstallMeta() {
    const filePath = getDefaultMetaPath();
    try {
        if (!fs.existsSync(filePath)) return null;
        const raw = fs.readFileSync(filePath, 'utf8');
        const parsed = JSON.parse(raw);
        if (!parsed || !parsed.binDir) return null;
        return parsed;
    } catch {
        return null;
    }
...
function installShims(toolDir, isWin) {
    ...
    // 5. 持久化 bin 目录元数据（uninstall 回退用）
    try {
        saveInstallMeta(binDir);
        details.push(`saved install meta: ${binDir}`);
    } catch (e) {
        details.push(`warning: failed to save install meta: ${e.message}`);
    }
...
function uninstallShims(toolDir, isWin) {
    ...
    // 1. 定位 bin 目录（先查 PATH，失败则回退到之前保存的元数据）
    ...
    if (binDir) {
        ...
        if (meta && meta.binDir) {
            binDir = meta.binDir;
            details.push(`using install meta (openspec not in PATH): ${binDir}`);
        } else {
            return {
                success: false,
                binDir: null,
                details: ['openspec CLI not found in PATH and no install meta available']
            };
        }
    }
    ...
    // 5. 清理安装元数据
    try {
        const metaPath = getDefaultMetaPath();
        if (fs.existsSync(metaPath)) {
            fs.unlinkSync(metaPath);
            details.push('cleaned up install meta');
        }
    } catch (e) {
        details.push(`warning: failed to clean up install meta: ${e.message}`);
    }

    return { success: true, binDir, details };
}

module.exports = { installShims, uninstallShims, findOpenspecBinDir, openspecOrigExists, copyOpenspecToOrig, installShimScripts, setExecutable, findOpenspecOrigFile, getRestoreTarget, getShimFiles, saveInstallMeta, loadInstallMeta, _setMetaPath };
